...
const NewTripForm = ({ users }) => {
    ...
    const canSave = [title, text, price, quantity, userId].every(Boolean) && !isLoading

    const onSaveTripClicked = async (e) => {
        e.preventDefault()
        if (canSave) {
            await addNewTrip({ user: userId, title, text, price, quantity })
        }
    }
        ...
        <>
            ...
            <form className="form" onSubmit={onSaveTripClicked}>
                ...
                <input
                    ...
                    min='1'
                    step={1}
                    className={`form__input form__input--price ${validQuantityClass}`}
                    id="quantity"
                    name="quantity"
                    value={quantity}
